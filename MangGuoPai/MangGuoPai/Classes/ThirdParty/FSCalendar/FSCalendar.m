    ...
    switch (self.placeholderType) {
        case FSCalendarPlaceholderTypeNone: {
            if (self.transitionCoordinator.representingScope == FSCalendarScopeMonth && monthPosition != FSCalendarMonthPositionCurrent) {
                ...
                FSCalendarBlankCell *cell = [self.dataSourceProxy calendar:self blankCellForDate:date atMonthPosition:monthPosition];
                if (!cell) {
                    cell = [self.collectionView dequeueReusableCellWithReuseIdentifier:FSCalendarBlankCellReuseIdentifier forIndexPath:indexPath];
                }
...
- (void)registerClass:(Class)cellClass forBlankCellReuseIdentifier:(NSString *)identifier
{
    if (!identifier.length) {
        [NSException raise:FSCalendarInvalidArgumentsExceptionName format:@"This identifier must not be nil and must not be an empty string."];
    }
    if (![cellClass isSubclassOfClass:[FSCalendarBlankCell class]]) {
        [NSException raise:@"The cell class must be a subclass of FSCalendarBlankCell." format:@""];
    }
    if ([identifier isEqualToString:FSCalendarBlankCellReuseIdentifier]) {
        [NSException raise:FSCalendarInvalidArgumentsExceptionName format:@"Do not use %@ as the cell reuse identifier.", identifier];
    }
...
- (FSCalendarBlankCell *)dequeueReusableBlankCellWithIdentifier:(NSString *)identifier forDate:(NSDate *)date atMonthPosition:(FSCalendarMonthPosition)position;
{
    
    NSIndexPath *indexPath = [self.calculator indexPathForDate:date atMonthPosition:position];
   
    FSCalendarBlankCell *cell = [self.collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    return cell;
}
