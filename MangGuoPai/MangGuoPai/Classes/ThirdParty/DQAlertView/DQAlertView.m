...
- (void)showInView:(UIView *)view
{
    ...
    if (self.shouldDimBackgroundWhenShowInView && view != window) {
        ...
        self.blackOpaqueView.backgroundColor = RGBA(0, 0, 0, self.dimAlpha);
...
- (void) addThisViewToView: (UIView *) view
{
    ...
    if (self.blackOpaqueView) {
        ...
        [UIView animateWithDuration:timeAppear animations:^{
            self.blackOpaqueView.alpha = self.dimAlpha;
        } completion:^(BOOL finished) {
...
- (void)calculateFrame
{
    ...
    if ( ! hasContentView ) {
        if ( ! hasModifiedFrame )
        {
            UIFont * messageFont = self.messageFont ?: PFSC(30);
