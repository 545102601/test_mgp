...
- (void)initView {
    ...
    _contentLabel.font = PFSC(30);
    _contentLabel.textColor = colorHex(@"#a2a2a2");
    ...
    _reloadButton.layer.borderColor = _contentLabel.textColor.CGColor;
    [_reloadButton setTitle:@"重新加载" forState:UIControlStateNormal];
    [_reloadButton setTitleColor:_contentLabel.textColor forState:UIControlStateNormal];
...
- (void)layoutSubviews {
    ...
    if (!_iconImageView.hidden) {
        CGFloat iconImageViewHeight = _iconImageViewSize.height ?: SW(180);
        CGFloat iconImageViewWidth = _iconImageViewSize.width ?: SW(180);
        _iconImageView.frame = CGRectMake(centerX - iconImageViewWidth * 0.5, subviewTop + _kViewMargin, iconImageViewWidth, iconImageViewHeight);
    ...
    if (!_contentLabel.hidden) {
        ...
        _contentLabel.frame = CGRectMake(kLabelMarginX, subviewTop + _kViewMargin, self.frame.size.width - 2* kLabelMarginX, [self calcuTextHeight]);
    ...
    if (!_reloadButton.hidden) {
        ...
        _reloadButton.frame = CGRectMake(centerX - reloadButtonWidht * 0.5, subviewTop + _kViewMargin, reloadButtonWidht, reloadButtonHeight);
        contentHeight = CGRectGetMaxY(_reloadButton.frame);
        
    }

    _contentView.frame = CGRectMake(0, 0, self.frame.size.width, contentHeight + _kViewMargin);
